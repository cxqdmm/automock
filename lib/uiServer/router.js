...
module.exports = (router) => {
    ...
    const filteredList = filters.reduce((result, filter) => {
      ...
      return result.filter((item) => {
        ...
        if (type === "[object String]") {
          return item[key] && ~item[key].indexOf(value);
  ...
  // 新增mock接口
  router.post("/cgi-bin/create-api-data", (ctx) => {
    const { localStorage } = ctx.req;
    const { name, data, pattern = "" } = ctx.request.body;
    try {
      writeFile({
      ...
      updateFile(localStorage, name, data);
      setProperty(localStorage, name, { mock: true });
      ctx.body = {
        code: 200,
        msg: "success",
      };
